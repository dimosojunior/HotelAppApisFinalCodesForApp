...
const HotelGuestOrdersDetailScreen = ({route ,navigation }) => {
...
const removeOrderItem = (cartId) => {
   ...
   // navigation.replace('View Hotel  Orders', {id}); 
    navigation.replace('All Hotel Closed Orders');
...
const closeBill = () => {
   navigation.replace('All Hotel Closed Orders');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/HotelOrderCloseBillByDeletingRoomOrder/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
export default HotelGuestOrdersDetailScreen;
