...
const RestaurantOrdersDetailScreen = ({route ,navigation }) => {
...
const [isRange, setisRange] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${EndPoint}/Cart/GetRestaurantOrderItems/?id=${id}`)
...
const removeOrderItem = (cartId) => {
   ...
   // navigation.replace('View Restaurant  Orders', {id}); 
    navigation.replace('All Restaurant Closed Orders');
  // Make an API request to delete the item from the cart
  const apiUrl = `${EndPoint}/Cart/RestaurantDeleteOrderItem/?id=${id}&cartId=${cartId}`;
...
const setOrderStatusTrue = () => {
  ...
  const url = `${EndPoint}/Cart/RestaurantOrderChangeStatusToTrue/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
//    navigation.replace('All Restaurant Closed Orders');
//   setModalVisibleProcessOrder(false);
//   const url = `${EndPoint}/Cart/RestaurantOrderChangeStatusToFalse/?id=${id}`;
//   fetch(url, {
//     method: 'POST',
//   })
//     .then((response) => {
...
const closeBill = () => {
   navigation.replace('All Restaurant Closed Orders');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/RestaurantOrderCloseBill/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
<MinorHeader title="Items"  screenName="Restaurant  Reports" />
...
export default RestaurantOrdersDetailScreen;
