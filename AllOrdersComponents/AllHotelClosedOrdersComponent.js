...
const AllHotelClosedOrdersComponent = () => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/SeeClosedAllHotelOrders/?page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
onPress={() => navigation.navigate('Hotel Closed Orders Detail Screen', item)} 
...
export default AllHotelClosedOrdersComponent;
