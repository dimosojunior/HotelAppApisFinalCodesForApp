...
const AllRetailsClosedOrdersComponent = () => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/SeeClosedAllRetailsOrders/?page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRetailsOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const ReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Retails  Orders', item)}
...
{item.closed_order_state === true && (
 ...
 {/*    MWISHO */}
     ...
     <Text style={[{
      color:'red',
      fontWeight:'Medium'
     },globalstyles.ReportScreentitle]}>Print Receipt</Text>
     </View>
...
onPress={() => navigation.navigate('Retails Closed Orders Detail Screen', item)} 
...
export default AllRetailsClosedOrdersComponent;
