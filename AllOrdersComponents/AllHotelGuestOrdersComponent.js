...
const AllHotelGuestOrdersComponent = () => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/SeeAllHotelGuestOrders/?page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
onPress={() => navigation.navigate('Hotel Guest Orders Detail Screen', item)}
...
export default AllHotelGuestOrdersComponent;
