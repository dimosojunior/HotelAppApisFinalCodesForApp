...
const AllRestaurantClosedOrdersComponent = () => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/SeeClosedAllRestaurantOrders/?page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRestaurantOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const ReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Restaurant  Orders', item)}
...
onPress={() => navigation.navigate('Restaurant Closed Orders Detail Screen', item)} 
...
export default AllRestaurantClosedOrdersComponent;
