...
const AllHotelGuestClosedOrdersComponent = () => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/SeeClosedAllHotelGuestOrders/?page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
onPress={() => navigation.navigate('Hotel Guest Closed Orders Detail Screen', item)}
...
export default AllHotelGuestClosedOrdersComponent;
