...
const HotelInventoryProductsCategoriesHomeScreen =({route, navigation }) => {
...
const AddFood =() =>{
 ...
 //FOR APIS
//const { datas:queryset,setDatas:setqueryset, isPending, error } = useFetch(`${EndPoint}'/PostData/PostHotelCategories/`);
...
const [queryset, setQueryset] = useState(null);
 ...
 useEffect(() => {
    ...
    const url = `${EndPoint}/Hotel/AllHotelCategories/?id=${id}`;
    //const url = `${EndPoint}/PostData/PostHotelCategories/`;
    axios.get(url)
...
<MinorHeader title={Category} pressMe={AddFood} screenName="Hotel Inventory" />
        ...
        renderItem={({item, index}) => {
          ...
          return (
           <>
            <TouchableOpacity 
             activeOpacity={1}
             onPress={() => navigation.navigate('Hotel Inventory Products', item)}
            >
...
export default HotelInventoryProductsCategoriesHomeScreen;
