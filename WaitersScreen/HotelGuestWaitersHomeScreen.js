...
const HotelGuestWaitersHomeScreen =({route, navigation }) => {
...
const [myWaiters, setMyWaiters] = useState([]);
  ...
  useEffect(() => {
   ...
   axios.get(`${EndPoint}/Cart/CountHotelGuestOrderForEachUser/?CategoryId=${CategoryId}`)
      .then((response) => {
              ...
              <View style={globalstyles.WaitersnameView}>
                ...
                <Text style={globalstyles.WaitersText}>Pending Orders: {item._pending_orders}</Text>
                 ...
                 style={globalstyles.WaitersactionBtn}
                  onPress={() => navigation.replace('Hotel Products Guest Reports', { ...item, CategoryId })}
                >
...
<MinorHeader title="Waiters" pressMe={AddBooked} screenName="Hotel Guest Reports"/>
...
{myWaiters && myWaiters.length > 0 ? (
 ...
 <View style={globalstyles.UnitcartCard}>
      ...
      {userData.username && 
       ...
       onPress={() => navigation.replace('Hotel Products Guest Reports', { ...userData.id, CategoryId })}
      >
        <View 
...
export default HotelGuestWaitersHomeScreen;
