...
const HotelWaitersHomeScreen =({route, navigation }) => {
  const { 
    id 
   } = route.params
...
const [myWaiters, setMyWaiters] = useState([]);
  ...
  useEffect(() => {
    ...
    // axios.get(EndPoint+'/Cart/CountHotelOrderForEachUser/')
    axios.get(`${EndPoint}/Cart/CountHotelOrderForEachUser/?CategoryId=${CategoryId}`)
      .then((response) => {
 ...
 if (input === ""){
    ...
    return (
      <View style={globalstyles.WaitersitemView}>
          ...
          onPress={() => navigation.replace('Hotel Products Reports', { ...item, CategoryId })}
        >
...
{myWaiters && myWaiters.length > 0 ? (
 ...
 <View style={globalstyles.UnitcartCard}>
      ...
      {userData.username && 
       ...
       <TouchableOpacity
     // onPress={() => navigation.replace('Hotel Products Reports', userData.id)} 
     onPress={() => navigation.replace('Hotel Products Reports', { ...userData.id, CategoryId })}
      >
        <View 
...
export default HotelWaitersHomeScreen;
