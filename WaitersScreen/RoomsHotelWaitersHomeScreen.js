...
const RoomsHotelWaitersHomeScreen =({navigation }) => {
...
useFocusEffect(
  ...
  const [isPending, setIsPending] = useState(true);
   ...
   //FOR APIS
// const { datas:queryset,setDatas:setQueryset, isPending, error } = useFetch(EndPoint+'/PostData/PostHotelWaiters/');
...
const [myWaiters, setMyWaiters] = useState([]);
  ...
  useEffect(() => {
    ...
    axios.get(EndPoint+'/Cart/CountHotelRoomsOrderForEachUser/')
      .then((response) => {
    ...
    return (
      <View style={globalstyles.WaitersitemView}>
              ...
              <View style={globalstyles.WaitersnameView}>
                ...
                <Text style={globalstyles.WaitersText}>Pending Orders: {item.Rooms_pending_orders}</Text>
                 <Text style={globalstyles.WaitersApprovedText}>Approved Orders: {item.Rooms_approved_orders}</Text>
              </View>
              
                 <TouchableOpacity
                 style={globalstyles.WaitersactionBtn}
                onPress={() => navigation.replace('Hotel Rooms Reports', item)} 
                >
...
<MinorHeader title="Waiters" pressMe={AddBooked} screenName="Hotel Reports"/>
...
{myWaiters && myWaiters.length > 0 ? (
 ...
 <View style={globalstyles.UnitcartCard}>
      ...
      {userData.username && 
       ...
       <TouchableOpacity
      onPress={() => navigation.replace('Hotel Rooms Reports', userData.id)} 
      >
        <View 
...
export default RoomsHotelWaitersHomeScreen;
