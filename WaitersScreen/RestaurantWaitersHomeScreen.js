...
const RestaurantWaitersHomeScreen =({route, navigation }) => {
  const { 
    id 
   } = route.params
...
useFocusEffect(
  ...
  const [isPending, setIsPending] = useState(true);
   ...
   //FOR APIS
// const { datas:queryset,setDatas:setQueryset, isPending, error } = useFetch(EndPoint+'/PostData/PostRestaurantWaiters/');
...
const [myWaiters, setMyWaiters] = useState([]);
  ...
  useEffect(() => {
   ...
   axios.get(`${EndPoint}/Cart/CountRestaurantOrderForEachUser/?CategoryId=${CategoryId}`)
      .then((response) => {
 ...
 if (input === ""){
    ...
    return (
      <View style={globalstyles.WaitersitemView}>
          ...
          onPress={() => navigation.replace('Restaurant Products Reports', { ...item, CategoryId })}
        >
...
<MinorHeader title="Waiters" pressMe={AddBooked} screenName="Restaurant Reports"/>
...
{myWaiters && myWaiters.length > 0 ? (
 ...
 <View style={globalstyles.UnitcartCard}>
      ...
      {userData.username && 
       ...
       <TouchableOpacity
     // onPress={() => navigation.replace('Restaurant Products Reports', userData.id)} 
     onPress={() => navigation.replace('Restaurant Products Reports', { ...userData.id, CategoryId })}
      >
        <View 
...
export default RestaurantWaitersHomeScreen;
