...
const RetailsProducts =({route, navigation}) => {
...
const HeaderName = [CategoryName];
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRetailsProducts/');
...
// State variable to store the RoomClasses data
 ...
 const [selectedProductUnit, setSelectedProductUnit] = useState(null);
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRetailsCategories/`)
      ...
      .catch((error) => {
        ...
        showAlertFunction("Error fetching Product categories");
      });
  }, []);


 useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRetailsProductsUnit/`)
      .then((response) => response.json())
      .then((data) => {
      ...
      .catch((error) => {
        ...
        showAlertFunction("Error fetching Product unit");
      });
  }, []);
 ...
 const handleSubmit = () => {
  if (
   ...
   showAlertFunction("All fields are required except product second name");
  } else if (
    ...
    showAlertFunction("Price and Product Quantity must be valid numbers");
  } else {
    ...
    fetch(EndPoint + '/PostData/PostAddRetailsProducts/', {
      method: 'POST',
      headers: {
      ...
      .then((data) => {
        ...
        showAlertFunction("Product added successfully");
    ...
    setQueryset((prevmyItems) => [ data, ...prevmyItems]);
      ...
      .catch((error) => {
        ...
        showAlertFunction("Failed to add product");
      });
  }
};
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Retails/RetailsProducts/?id=${id}&page=${current_page}&page_size=8` 
...
//     if (shouldReload) {
//       navigation.replace('Retails NewSale Pizza');
...
useEffect(() => {
  ...
  const fetchTables = async () => {
    try {
      const response = await axios.get(EndPoint + '/PostData/PostRetailsTables/', {
        // headers: {
      ...
      showAlertFunction("Error fetching tables");
    }
  };
...
useEffect(() => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get(EndPoint + '/PostData/PostRetailsCustomers/', {
        // headers: {
        //   Authorization: `Token ${userToken}`,
        // },
      });
      ...
      showAlertFunction("Error fetching customers");
    }
  };
...
navigation.replace('Retails Products', { id }); 
  ...
  try {
    const response = await axios.post(
     ...
     //setShouldReload(true); 
      //navigation.replace('Retails NewSale Other '); 
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/RetailsDeleteCartItem/?cartId=${cartId}`;
    ...
    .then((response) => {
      if (response.status === 204) {
        ...
        setCart(cart.filter((item) => item.id !== cartId));
         ...
         // navigation.replace('Retails Products');  
...
const makeOrder = async () => {
  ...
  if (!selectedTable) {
    ...
    showAlertFunction("Please all fields are required");
    return;
  }
  //navigation.replace('Retails Products');
...
const [modalVisibleAddNewCustomer, setModalVisibleAddNewCustomer] = useState(false);
 ...
 const handleAddCustomer = () => {
  ...
  if (
    ...
    showAlertFunction("Enter valid Phone number");
  } else {
    // Validation passed, make the API request
...
const formData = {
    ...
    fetch(EndPoint + '/PostData/PostRetailsCustomers/', {
      ...
      .then((data) => {
        ...
        showAlertFunction("Customer added successfully");
    ...
    setCustomerClasses((prevmyItems) => [ data, ...prevmyItems]);
      ...
      .catch((error) => {
       ...
       showAlertFunction("Failed to add Customer");
      });
  }
};
...
<AddMinorHeader title={HeaderName} pressMe={Add} screenName="Retails  Walking Customers" />
...
{/*--------ADD MORE INFORMATIONS WHEN MAKING AN ORDER-------------*/}
    ...
    <ScrollView keyboardShouldPersistTaps="handled">
        <View style={{ flex: 1,marginTop:height/5, justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' }}>
          <View style={globalstyles.ModalView}>
            <Text 
            style={globalstyles.ViewOrderModalTitle}>MAKE NEW ORDER</Text>
     ...
     <View style={globalstyles.picker}>     
          ...
          >
            ...
            <View style={globalstyles.ButtonConatiner}>
                    ...
                    style={globalstyles.ButtonAdd}  
                    onPress={makeOrder} >
                        <Text  style={globalstyles.ConfirmCancelButtonText}
                        >CONFIRM</Text>
                    </TouchableOpacity>
            </View>
          </View>
...
{/*mwisho wa form*/}
  ...
  <View style={globalstyles.ButtonConatiner}>
                    ...
                    onPress={() => setModalVisibleAddNewCustomer(false)} >
                        <Text 
                         style={globalstyles.ConfirmCancelButtonText}
                         >CLOSE</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                    style={globalstyles.ButtonAdd}  
...
{/*MODAL FOR PRINTING*/}
      ...
      >
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' }}>
          <View style={globalstyles.ModalView}>
            <Text 
            style={globalstyles.ViewOrderModalTitle}
            >VIEW ORDER</Text>
...
export default RetailsProducts;
