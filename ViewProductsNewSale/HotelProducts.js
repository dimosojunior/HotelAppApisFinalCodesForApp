...
const HotelProducts =({route, navigation}) => {
...
const HeaderName = [CategoryName];
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostHotelProducts/');
...
// State variable to store the RoomClasses data
 ...
 const [selectedProductUnit, setSelectedProductUnit] = useState(null);
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostHotelCategories/`)
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostAddHotelProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Hotel/HotelProducts/?id=${id}&page=${current_page}&page_size=8` 
...
const addCartItem = async () => {
  ...
  if (parseInt(quantity) > selectedProduct.ProductQuantity) {
    ...
    showAlertFunction("Not enough quantity in a stock");
    return;
  }
navigation.replace('Hotel Products', { id });  
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/HotelDeleteCartItem/?cartId=${cartId}`;
    ...
    .then((response) => {
      if (response.status === 204) {
        ...
        setCart(cart.filter((item) => item.id !== cartId));
         ...
         // navigation.replace('All Hotel Walking ');  
...
const makeOrderForGuestCustomer = async () => {
  ...
  //navigation.replace('All Hotel Guest ');
  //call the print Receipt function
     navigation.replace('Hotel Products', { id }); 
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/PlusRoom_MakeHotelOrderWithRoom/',
      { 
        total_price: totalCartPrice,
        table: selectedTable, // Include the selected table
        // CustomerFullName: CustomerFullName,
...
<AddMinorHeader title={HeaderName} pressMe={Add} screenName="Hotel  Walking Customers" />
...
{/*--------ADD MORE INFORMATIONS WHEN MAKING AN ORDER-------------*/}
  ...
  <View style={globalstyles.swapButtonsheader}>

  <Text 
  style={globalstyles.swapButtonsheaderText}
...
{/*mwanzo wa swapButtonsContainer*/}
 ...
 <View style={globalstyles.swapButtonsContainer}>
  ...
  <View style={globalstyles.customSwitchContainer}>
   ...
   options={[
      { label: "Guest Customer", value: false },
      { label: "Walking Customer", value: true },
    ]}
...
{/*mwisho wa swapButtonsContainer*/}
 ...
 <View style={globalstyles.swapButtonsContentsContainer}>
{ showhide !== true ? (
     ...
     <View style={globalstyles.picker}>     
          ...
          >
            ...
            <View style={globalstyles.ButtonConatiner}>
                    ...
                    <TouchableOpacity 
                    style={globalstyles.ButtonAdd}  
                    onPress={makeOrderForGuestCustomer} >
                        <Text style={globalstyles.ConfirmCancelButtonText}>CONFIRM</Text>
                    </TouchableOpacity>
            </View>
          </View>
