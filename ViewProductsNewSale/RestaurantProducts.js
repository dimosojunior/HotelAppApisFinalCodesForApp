...
const RestaurantProducts =({route, navigation}) => {
...
const HeaderName = [CategoryName];
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRestaurantProducts/');
...
// State variable to store the RoomClasses data
 ...
 const [selectedProductUnit, setSelectedProductUnit] = useState(null);
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRestaurantCategories/`)
 ...
 useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRestaurantProductsUnit/`)
      .then((response) => response.json())
      .then((data) => {
 ...
 const handleSubmit = () => {
  if (
   ...
   showAlertFunction("All fields are required except product second name");
    ...
    fetch(EndPoint + '/PostData/PostAddRestaurantProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Restaurant/RestaurantProducts/?id=${id}&page=${current_page}&page_size=8` 
...
useEffect(() => {
  ...
  const fetchTables = async () => {
    try {
      const response = await axios.get(EndPoint + '/PostData/PostRestaurantTables/', {
        // headers: {
...
useEffect(() => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get(EndPoint + '/PostData/PostRestaurantCustomers/', {
        // headers: {
        //   Authorization: `Token ${userToken}`,
        // },
      });
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/RestaurantDeleteCartItem/?cartId=${cartId}`;
...
<AddMinorHeader title={HeaderName} pressMe={Add} screenName="Restaurant  Walking Customers" />
