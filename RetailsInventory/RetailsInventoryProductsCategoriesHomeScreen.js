...
const RetailsInventoryProductsCategoriesHomeScreen =({route, navigation }) => {
...
const AddFood =() =>{
 ...
 //FOR APIS
//const { datas:queryset,setDatas:setqueryset, isPending, error } = useFetch(`${EndPoint}'/PostData/PostRetailsCategories/`);
...
const [queryset, setQueryset] = useState(null);
 ...
 useEffect(() => {
    ...
    const url = `${EndPoint}/Retails/AllRetailsCategories/?id=${id}`;
    //const url = `${EndPoint}/PostData/PostRetailsCategories/`;
    axios.get(url)
...
<MinorHeader title={Category} pressMe={AddFood} screenName="Retails Inventory" />
        ...
        renderItem={({item, index}) => {
          ...
          return (
           <>
            <TouchableOpacity 
             activeOpacity={1}
             onPress={() => navigation.navigate('Retails Inventory Products', item)}
            >
...
export default RetailsInventoryProductsCategoriesHomeScreen;
