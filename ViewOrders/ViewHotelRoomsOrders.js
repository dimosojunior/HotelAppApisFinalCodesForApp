...
const closeBill = () => {
  navigation.replace('Rooms Hotel Waiters');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/HotelRoomsOrderCloseBill/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
const removeOrderItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/HotelRoomsDeleteOrderItem/?id=${id}&cartId=${cartId}`;
      ...
      >
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.2)' }}>
          <View style={globalstyles.ModalView}>
            <Text style={{ marginLeft:90,fontFamily:'Medium', }}>ORDER RECEIPT</Text>
