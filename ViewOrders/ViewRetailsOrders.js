...
const ViewRetailsOrders = ({route ,navigation }) => {
...
const move =() =>{
  ...
  navigation.navigate('Retails Reports');
}
...
const [isRange, setisRange] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${EndPoint}/Cart/GetRetailsOrderItems/?id=${id}`)
...
const removeOrderItem = (cartId) => {
   ...
   // navigation.replace('View Retails  Orders', {id}); 
   navigation.replace('Retails Reports');
  // Make an API request to delete the item from the cart
  const apiUrl = `${EndPoint}/Cart/RetailsDeleteOrderItem/?id=${id}&cartId=${cartId}`;
...
const setOrderStatusTrue = () => {
  ...
  const url = `${EndPoint}/Cart/RetailsOrderChangeStatusToTrue/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
      if (response.ok) {
        ...
        showAlertFunction("Order processed successfully");
...
//   navigation.replace('Retails Reports');
//   setModalVisibleProcessOrder(false);
//   const url = `${EndPoint}/Cart/RetailsOrderChangeStatusToFalse/?id=${id}`;
//   fetch(url, {
//     method: 'POST',
//   })
//     .then((response) => {
...
const closeBill = () => {
  navigation.replace('Retails Reports');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/RetailsOrderCloseBill/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
      if (response.ok) {
        ...
        showAlertFunction("Bill Closed successfully");
      } else {
        // Request failed with a status code other than 200
...
<MinorHeader title="Items"  screenName="Retails  Reports" />
...
{/*MODAL FOR PROCESSING ORDERS*/}
      ...
      >
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.2)' }}>
          <View style={globalstyles.ModalView}>
          {order_status === false ? (
            <Text style={{ marginLeft:90,fontFamily:'Medium', }}>         
            PROCESS ORDER</Text>):(
            <Text style={{ marginLeft:90,fontFamily:'Medium', }}>         
            CLOSE BILL</Text>
            )}
...
export default ViewRetailsOrders;
