...
const UserRetailsDrinksReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/RetailsDrinksOrder/?page=${current_page}&page_size=2`;
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRetailsDrinksOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
<Text style={[{
  marginLeft:20,
},globalstyles.CartListHeader]}>Drinks Report</Text>
...
export default UserRetailsDrinksReportsComponent;
