...
const UserRetailsOthersReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/RetailsOthersOrder/?page=${current_page}&page_size=2`;
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRetailsOthersOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
<Text style={[{
  marginLeft:20,
},globalstyles.CartListHeader]}>Other Products Report</Text>
...
export default UserRetailsOthersReportsComponent;
