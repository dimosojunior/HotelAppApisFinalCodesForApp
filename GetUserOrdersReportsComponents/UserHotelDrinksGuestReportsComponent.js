...
const UserHotelDrinksGuestReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/HotelGuestDrinksOrdersForSpecificUser/?page=${current_page}&page_size=2`;
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterHotelDrinksOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
{item.closed_order_state === true && (
 ...
 {/*    MWANZO */}
     ...
     <View style={globalstyles.ReportScreenIndividualItemLeftcontainer}>
     <Text style={globalstyles.ReportScreentitle}>Drinks Order Price:</Text>
     </View>
...
export default UserHotelDrinksGuestReportsComponent;
