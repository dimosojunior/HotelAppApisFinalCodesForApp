...
const HotelDrinksGuestReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/HotelGuestDrinksOrderReport/?id=${id}&page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterHotelGuestDrinksOrderReport/?id=${id}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const DrinksReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Hotel Drinks Orders', item)}
...
{/*    MWANZO */}
     ...
     <View style={globalstyles.ReportScreenIndividualItemLeftcontainer}>
     <Text style={globalstyles.ReportScreentitle}>Cost of Drinks:</Text>
     </View>
...
onPress={() => navigation.navigate('View Hotel Guest Drinks Orders', item)} 
...
<Text style={globalstyles.CartListHeader}>Drinks Report</Text>
...
export default HotelDrinksGuestReportsComponent;
