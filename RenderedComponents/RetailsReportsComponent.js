...
const RetailsReportsComponent = ({id,CategoryId, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/RetailsOrderReport/?id=${id}&CategoryId=${CategoryId}&page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRetailsOrderReport/?id=${id}&CategoryId=${CategoryId}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
export default RetailsReportsComponent;
