...
const HotelGuestReportsHomeScreenComponent =({inventory}) => {
  ...
  const renderItem = ({item, index}) => {
    ...
    return (
       ...
       activeOpacity={1}
         ...
         //onPress = {() => navigation.navigate('Hotel Guest Waiters', item)}
         onPress={() => {
          
          if (item.Category === 'Rooms') {
            navigation.navigate('Rooms Hotel Waiters');
          } else {
            navigation.navigate('Hotel Guest Waiters', item);
          }
        }}
...
export default HotelGuestReportsHomeScreenComponent;
