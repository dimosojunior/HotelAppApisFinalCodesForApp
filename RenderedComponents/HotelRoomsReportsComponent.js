...
const HotelRoomsReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/HotelRoomsOrderReport/?id=${id}&page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterHotelRoomsOrderReport/?id=${id}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const RoomsReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Hotel Rooms Orders', item)}
...
{orders && orders.length > 0 ? (
 ...
 <View style={globalstyles.CartListHeaderContainer}>
  ...
  <Text style={globalstyles.CartListHeader}>Rooms Report</Text>
  {isRange ? (
 ...
 {setLoading===true?(<ActivityIndicator/>):(
  ...
  <Text style={globalstyles.NoProductText}>No any Room report
...
export default HotelRoomsReportsComponent;
