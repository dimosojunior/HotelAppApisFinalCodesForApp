...
const HotelOthersGuestReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/HotelGuestOthersOrderReport/?id=${id}&page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterHotelGuestOthersOrderReport/?id=${id}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const OthersReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Hotel Others Orders', item)}
...
onPress={() => navigation.navigate('View Hotel Guest Others Orders', item)} 
...
<Text style={globalstyles.CartListHeader}>Other Products Report</Text>
 ...
 {setLoading===true?(<ActivityIndicator/>):(
  ...
  <Text style={globalstyles.NoProductText}>No any products report
...
export default HotelOthersGuestReportsComponent;
