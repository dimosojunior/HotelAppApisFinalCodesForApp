...
const HotelGuestReportsComponent = ({id,CategoryId, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/HotelGuestOrderReport/?id=${id}&CategoryId=${CategoryId}&page=${current_page}&page_size=2`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterHotelGuestOrderReport/?id=${id}&CategoryId=${CategoryId}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
onPress={() => navigation.navigate('View Hotel Guest Orders', item)} 
...
<Text style={globalstyles.CartListHeader}> Report</Text>
...
export default HotelGuestReportsComponent;
